...
const Signup: React.FC<SignupProps> = ({ onBack, onLogin, onSuccess }) => {
  ...
  const handleSignupSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await mockBackend.signup(formData);
      if (result.success) {
  ...
  const handleVerifySubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Step 2: Verify Code
      const result = await mockBackend.verifyEmail(verificationCode);
      if (result.success) {
  ...
  const handlePaymentSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        const result = await mockBackend.setPaymentMethod(selectedPayment);
        if (result.success && result.user) {
            ...
            setError('Failed to process payment setup. Please try again.');
        }
    } catch (err) {
  ...
  const handleResendCode = async () => {
    ...
    await mockBackend.resendVerificationCode(formData.email);
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      setError('Google sign-up failed. Please use email instead.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const renderPaymentOption = (id: 'apple' | 'google' | 'card', label: string, icon: React.ReactNode) => (
    <label className={`flex items-center p-4 border rounded-xl cursor-pointer transition-all duration-200 ${
        ...
        <div className={`w-5 h-5 rounded-full border flex items-center justify-center mr-4 ${
            selectedPayment === id ? 'border-indigo-500 bg-indigo-500 text-white' : 'border-slate-500 bg-transparent'
        }`}>
        ...
        <div className="text-center mb-10">
          ...
          {step === 'verification' && (
             <>
              ...
              <p className="text-slate-400">Code sent to <span className="text-white font-medium">{formData.email}</span></p>
            </>
          )}
           {step === 'payment' && (
             <>
              ...
              <form className="space-y-4" onSubmit={handleSignupSubmit}>
                ...
                <div>
                  ...
                  <input 
                    ...
                    className={`w-full px-4 py-3 bg-slate-950 border rounded-lg text-white placeholder-slate-500 focus:outline-none focus:ring-1 transition-colors ${
                       error && error.includes('email') ? 'border-red-500 focus:border-red-500 focus:ring-red-500' : 'border-slate-800 focus:border-indigo-500 focus:ring-indigo-500'
                    }`}
                ...
                <div>
                  ...
                  <input 
                    ...
                    onChange={handleInputChange}
                    className={`w-full px-4 py-3 bg-slate-950 border rounded-lg text-white placeholder-slate-500 focus:outline-none focus:ring-1 transition-colors ${
                      error && error.includes('Password') ? 'border-red-500 focus:border-red-500 focus:ring-red-500' : 'border-slate-800 focus:border-indigo-500 focus:ring-indigo-500'
                   }`}
            ...
            <form className="space-y-6" onSubmit={handleVerifySubmit}>
              <div className="flex flex-col items-center">
                ...
                <p className="text-sm text-slate-400 text-center mb-6">
                  Please enter the code sent to your inbox. <br/>
                  <span className="text-xs text-indigo-400 mt-1 block">(Hint: Use 123456)</span>
                </p>
              </div>

              <div>
                <label className="block text-sm font-medium text-slate-300 mb-1.5 text-center">Verification Code</label>
                <input 
                  type="text" 
                  maxLength={6}
                  ...
                  className={`w-full text-center text-2xl tracking-widest px-4 py-3 bg-slate-950 border rounded-lg text-white placeholder-slate-700 focus:outline-none focus:ring-1 transition-colors ${
                  ...
                  autoFocus
                />
                {error && <p className="text-red-400 text-xs text-center mt-2 animate-fade-in">{error}</p>}
              </div>
