...
interface PaymentModalProps {
    ...
    plan: string;
    mode?: 'payment' | 'setup'; // Default to 'payment'
}

const CheckoutForm = ({ onClose, onSuccess, clientSecret, mode }: { onClose: () => void, onSuccess?: () => void, clientSecret: string, mode: 'payment' | 'setup' }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (clientSecret.startsWith('pi_')) {
            ...
            // Setup Intent (Trial Subscription / Update Card)
        ...
        if (error) {
            ...
            if (clientSecret.startsWith('pi_')) {
                ...
                setMessage(mode === 'setup' ? "Payment method updated!" : "Trial activated! You wont be charged for 14 days.");
            }
    ...
    return (
        <form id="payment-form" onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex justify-between items-center mt-4">
                ...
                >
                    <span id="button-text">
                        {isLoading ? <div className="spinner" id="spinner">Processing...</div> : (mode === 'setup' ? "Save Card" : "Start Trial")}
                    </span>
                </button>
            </div>
...
export default function PaymentModal({ isOpen, onClose, onSuccess, plan, mode = 'payment' }: PaymentModalProps) {
        ...
        if (isOpen) {
            ...
            const fetchClientSecret = async () => {
                try {
                    ...
                    if (mode === 'setup') {
                        // Invoke manage-subscription for Setup Intent (Update Card)
                        ...
                        // Standard Subscription Flow (Legacy payment-sheet)
                        const { data, error } = await supabase.functions.invoke('payment-sheet', {
                            body: {
                                plan: plan,
                                email: user?.email,
                                user_id: user?.id,
                                voiceflow_user_id: user?.user_metadata?.voiceflow_id
                            }
                        });
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md p-6 shadow-2xl relative overflow-hidden">
                ...
                <h2 className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-white to-slate-400 mb-6 text-center">
                    {mode === 'setup' ? 'Update Payment Method' : 'Secure Payment'}
                </h2>

                {clientSecret ? (
                    <Elements options={options} stripe={stripePromise}>
                        <CheckoutForm onClose={onClose} onSuccess={onSuccess} clientSecret={clientSecret} mode={mode} />
                    </Elements>
                ) : (
